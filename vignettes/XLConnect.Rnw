...
{\small\urlwofont{http://mirai-solutions.com}}\\[-0.3cm]
{\small\urlwofont{http://cran.r-project.org/web/packages/XLConnect/index.html}}
}
...
\includegraphics[width=11cm]{img/XLConnectExample1.png}
\end{center}
\end{figure}
...
\includegraphics[width=10cm]{img/XLConnectExample3.png}
\end{center}
\end{figure}
...
\subsection{Generating an Excel report of currency exchange rates development}
In this example, we will show an example use case of \pkg{XLConnect}: generation of 
a simple Excel report, showing the recent development of currency exchange rates 
(EUR, USD, GBP, JPY vs Swiss Franc (CHF)).

For this example we are going to also use some other packages:
\begin{itemize}
	\item \pkg{fImport}: Rmetrics - Economical and Financial Data Import
	\item \pkg{forecast}: Forecasting functions for time series
	...
	\item \pkg{scales}: Provides methods for automatically determining breaks and labels for axes and legends
\end{itemize}
...
The figure below illustrates the first few rows of the named region we have 
written to the Excel file. Please note, the data in your file will look different,
since you will be running the example at a different time.

\begin{figure}[htb]
\begin{center}
\caption{currency named region we have written}
\includegraphics[width=12cm]{img/AdvancedExample1a.png}
\end{center}
\end{figure}

\newpage Now, we are going to work on the report further, to make it {\bf look} nicer.
Let's say that for each currency, we want to highlight the points in time when 
there was a change of more than 2\% compared to the previous day.
...
The illustration below shows the updated swiss\_franc.xlsx file, with highlighted cells,
where day-to-day change was bigger than 2\%. Please note, the data in your file will look different,
since you will be running the example at a different time.

\begin{figure}[htb]
\begin{center}
\caption{Highlighted cells, where day-to-day change bigger than 2\%}
\includegraphics[width=12cm]{img/AdvancedExample1b.png}
\end{center}
\end{figure}

\newpage Further, we will add a graph to our report.
...
As a result, we obtain the following graph, written into Excel file as 
"graph" named region. Please note, your graph will look different, since
you will be running the code at a different time.

\begin{figure}[htb]
\begin{center}
\caption{Graph showing the currency trends}
\includegraphics[width=15cm]{img/AdvancedExample1c.png}
\end{center}
\end{figure}

% As the final step to producing our report, let's enrich it with some formulae!
% We will add the maximum downturn (= minimum) for the currencies at the bottom
...
\begin{itemize}
	\item{Ability to read cached cell values. There is a new argument useCachedValues
	...
	\item{There are new arguments keep and drop to readWorksheet and readNamedRegion
  ...
  \item{String conversions when reading in data from Excel (readWorksheet, readNamedRegion)
    now respect the data format of the cells}
  \item{Support for numeric missing value identifiers (see method setMissingValue)}
  \item{New Vignette "XLConnect for the Impatient" to get people started quickly}
  \item{Added a new function getLastColumn to query the last non-empty column in a
    worksheet}
  \item{The bounding box detection algorithm for readWorksheet has been adapted to better
  ...
  \item{Fixed an issue with writing empty data.frames}
  \item{Moved to Apache POI 3.9 (see package XLConnectJars)}
  \item{Ensured vignette compatibility with ggplot2 0.9.3}
\end{itemize}

A summary of news in \pkg{XLConnect} \Sexpr{getOption("XLConnect.Version")} can be found in the 
\href{http://cran.r-project.org/web/packages/XLConnect/NEWS}{NEWS} file on CRAN.
...
Therefore, you need to set the Java heap size in Eclipse's "Run Configurations" (Run > Run 
Configurations on top menu) for the R engine:\\

\begin{figure}[htb]
	\begin{center}
		\caption{Setting the Java heap size in Eclipse: }
		\includegraphics[width=16.1cm]{img/RunConfigurations.png}
	\end{center}
\end{figure}
...
\href{mailto:xlconnect-bugs@mirai-solutions.com}{xlconnect-bugs@mirai-solutions.com}
\end{minipage}
...
\urlwofont{http://www.linkedin.com/company/mirai-solutions-gmbh}
\end{minipage}

\begin{minipage}[c]{0.08\textwidth}
\includegraphics[width=10mm, height=10mm]{img/TwitterLogo.png}
\end{minipage}
\begin{minipage}[c]{0.9\textwidth}
We encourage you to also follow us on {\bf Twitter}: \\
\href{http://twitter.com/miraisolutions}{@miraisolutions}
\end{minipage}
...
\begin{enumerate}
	\item	Mirai Solutions website: 
			\\\urlwofont{http://www.mirai-solutions.com}
	\item 	Mirai Solutions blog on Wordpress: 
			\\\urlwofont{http://miraisolutions.wordpress.com/}
	\item 	Mirai Solutions LinkedIn Page: 
			\\\urlwofont{http://www.linkedin.com/company/mirai-solutions-gmbh}
	\item 	\pkg{XLConnect} on cran: 
			\\\urlwofont{http://cran.r-project.org/web/packages/XLConnect/index.html}
	\item 	\pkg{XLConnect} on crantastic!: 
			\\\urlwofont{http://crantastic.org/packages/XLConnect}
	\item 	\pkg{XLConnect} on inside-R!: 
			\\\urlwofont{http://www.inside-r.org/packages/xlconnect}
	\item	The R-Podcast Episode 6: Importing Data from External Sources:
			\\\urlwofont{http://www.r-podcast.org/the-r-podcast-episode-6-importing-data-from-external-sources/}
	\item 	R Base course by Dr. Roberto Dantas de Pinho, referring to \pkg{XLConnect} (Portuguese):
			\\\urlwofont{http://www.slideshare.net/RobertodePinho/curso-bsico-de-r}
	\item 	YouTube tutorial by bebilda, on importing and exporting data using \pkg{XLConnect} (Spanish): 
			\\\urlwofont{http://www.youtube.com/watch?v=84ShWUiq5Mg}	
	\item 	YouTube tutorial by MrIanfellows, on importing data into R using JGR and \pkg{XLConnect}:
			\\\urlwofont{http://www.youtube.com/watch?v=1QA_7ks1pnc} 
	\item 	\pkg{XLConnect} usage described in the R Programming/Data Management section in Wikibooks 
			\\(Reading and saving data > Importing and exporting data > Excel (xls,xlsx)):
			\\\urlwofont{http://en.wikibooks.org/wiki/R_Programming/Data_Management}		
	\item 	"How to Read Data from Excel into R", article by Joris Meys and Andrie de Vries:
			\\\urlwofont{http://www.dummies.com/how-to/content/how-to-read-data-from-excel-into-r.html} 		
	\item 	"R for Dummies", book by Joris Meys and Andrie de Vries, page 211:
			\\\urlwofont{http://tinyurl.com/c54e35m}
	\item 	"Grundlagen der Datenanalyse mit R: Eine anwendungsorientierte Einfuhrung", book by Daniel Wollschläger:
			\\\urlwofont{http://books.google.ch/books?id=tlmcPQ9YZP4C&printsec=frontcover&source=gbs_ge_summary_r&cad=0\#v=onepage&q&f=false}
...
% Bye bye box
\tikzDocDisclaim{Thanks for giving XLConnect a try!\vspace{2mm}}
{
\vskip\baselineskip

We hope that you enjoy using \pkg{XLConnect}. We would be very happy to hear your feedback about
the package and about this Vignette.

\vspace{3mm}

Also, we would be very grateful if you were willing to spare a few minutes and rate the package, or write a 
review, on \href{http://crantastic.org/packages/XLConnect}{crantastic!}

\vspace{3mm}

\textit{- Mirai Solutions -} 
